...
class SourceGenerator(NodeVisitor):
    ...
    def value_type(self, value):
        if isinstance(value,ast.Num):
            ...
            value_type = str(type(value.elts[0].value))
            type_name = value_type.split("'")[1]
            #return type_name
            return 'array['+type_name+','+str(len(value.elts))+']'
        else:
            return 'value'
            #return str(type(value))
    def visit_Assign(self, node):
        self.newline(node)
        self.write(self.value_type(node.value))
        self.write(' ')
        for idx, target in enumerate(node.targets):
            ...
            if isinstance(target, ast.Name):
                 game = 1#print("Variable:", target.id, "Type:", end=' ')
        
            self.visit(target)
        self.write(" = ")
        self.visit(node.value)
        self.write(';') #add ';' after all assignment statement

    def visit_AugAssign(self, node):
        self.newline(node)
        self.write(self.value_type(node.value))
        #type1 = type(node.value)
        self.write(" ")
    ...
    def visit_FunctionDef(self, node):
        ...
        for statement in node.body:
            if isinstance(statement, ast.Return):
                ...
                break  # Exit loop after finding the first return statement
        self.write(')')
        self.write(' -> value ')
        #self.write(return_type)
        self.write('{')
        self.body(node.body)
        self.newline(extra=0)
        self.write('}')
    ...
    def get_type(self,node):
        if isinstance(node.value, ast.Num) and isinstance(node.value.n, float):
            self.write("float ")
